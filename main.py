...
from flask import Flask, request, jsonify, send_file, g, redirect, session
...
YOUTUBE_CLIENT_SECRETS_FILE = os.path.join(EXEC_DIR, 'client_secrets.json')
YOUTUBE_SCOPES = ['https://www.googleapis.com/auth/youtube.upload']
YOUTUBE_TOKENS_DB = os.path.join(EXEC_DIR, 'youtube_tokens.json')

# Store for pending OAuth states
YOUTUBE_OAUTH_STATES = {}

def load_youtube_tokens():
    """Load stored YouTube tokens"""
    try:
        if os.path.exists(YOUTUBE_TOKENS_DB):
            with open(YOUTUBE_TOKENS_DB, 'r') as f:
                return json.load(f)
    except:
        pass
    return {}

def save_youtube_tokens(tokens):
    """Save YouTube tokens"""
    try:
        with open(YOUTUBE_TOKENS_DB, 'w') as f:
            json.dump(tokens, f)
    except Exception as e:
        print(f"Error saving YouTube tokens: {e}")

def get_youtube_credentials(session_id):
    """Get YouTube credentials for a session"""
    tokens = load_youtube_tokens()
    ...
    if creds.expired and creds.refresh_token:
        try:
            ...
            # Update stored token
            tokens[session_id]['token'] = creds.token
            save_youtube_tokens(tokens)
        except Exception as e:
...
def get_youtube_auth_url():
    ...
    if not os.path.exists(YOUTUBE_CLIENT_SECRETS_FILE):
        return jsonify({
            'ok': False, 
            'error': 'client_secrets.json not found. Please add your YouTube API credentials.',
            'needsSetup': True
        })
    ...
    try:
        ...
        # Set redirect URI (must match Google Cloud Console)
        # Try to get the host from request or use default
        host = request.host_url.rstrip('/')
        flow.redirect_uri = f"{host}/api/youtube/callback"
        
        # Generate state for security
        state = str(uuid.uuid4())
        YOUTUBE_OAUTH_STATES[state] = session_id
        ...
        print(f"YouTube auth URL error: {e}")
        traceback.print_exc()
...
def youtube_callback():
    ...
    # Verify state
    session_id = YOUTUBE_OAUTH_STATES.pop(state, None)
    if not session_id:
...
def youtube_status():
    """Check if YouTube is connected for this session"""
    session_id = request.args.get('sessionId')
    ...
    if not os.path.exists(YOUTUBE_CLIENT_SECRETS_FILE):
        return jsonify({
            ...
            'connected': False,
            'needsSetup': True,
            'message': 'YouTube API not configured. Add client_secrets.json'
        })
    ...
    return jsonify({
        ...
        'needsSetup': not os.path.exists(YOUTUBE_CLIENT_SECRETS_FILE)
    })
...
def upload_to_youtube():
    ...
    session_id = d.get('sessionId')
    job_id = d.get('jobId')
    title = d.get('title', 'Quran Video')
    description = d.get('description', '')
    tags = d.get('tags', ['قرآن', 'Quran', 'إسلام'])
    privacy = d.get('privacy', 'unlisted')  # public, unlisted, private
    
    if not session_id or not job_id:
        return jsonify({'ok': False, 'error': 'sessionId and jobId required'}), 400
    
    # Get credentials
    creds = get_youtube_credentials(session_id)
    if not creds or not creds.valid:
        return jsonify({'ok': False, 'error': 'YouTube not connected', 'needsAuth': True}), 401
    ...
    try:
        ...
        body = {
            'snippet': {
                ...
                'tags': tags,
                'categoryId': '29'  # Nonprofits & Activism category
            },
            'status': {
