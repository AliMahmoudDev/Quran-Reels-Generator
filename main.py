...
from pydub import AudioSegment, silence  # ✅ Added 'silence' for the new estimator
...
# 🚀 Reciters Config (Existing)
NEW_RECITERS_CONFIG = {
    'إسلام صبحي': (253, "https://server14.mp3quran.net/islam/"),
...
# 🆕 Full Surah Reciters (Add your new reciters here)
...
# 🆕 New Function: Estimate Ayah Timings from Full Audio
def estimate_ayah_timings(audio_path, ayah_texts, silence_thresh_offset=-16, min_silence_len=700):
    """
    Hybrid estimation: Silence Detection + Text Length Ratio
    """
    ...
    if num_chunks == num_ayahs:
        ...
        for i, (start_ms, end_ms) in enumerate(chunks_ms):
            ...
            # Extend to next start for smooth transitions
            if i < num_chunks - 1:
...
# ==========================================
# ⚡ Optimized Video Builder (Segmented + Full Surah Support)
# ==========================================
def build_video_task(job_id, user_pexels_key, reciter_id, surah, start, end, quality, bg_query, fps, dynamic_bg, use_glow, use_vignette):
    ...
    try:
        ...
        # ==================================================
        # 🆕 CHECK: Full Surah Logic vs. API Logic
        # ==================================================
        is_full_surah_mode = reciter_id in FULL_SURAH_RECITERS
        sliced_audio_map = {}
        
        if is_full_surah_mode:
            update_job_status(job_id, 5, "Downloading Full Surah & Estimating Timings...")
            
            # A. Prepare Texts
            all_texts = []
            for a in range(start, last+1):
                 all_texts.append(get_text(surah, a))
            
            # B. Download Full Audio
