...
SURAH_NAMES = ['Ø§Ù„ÙØ§ØªØ­Ø©', 'Ø§Ù„Ø¨Ù‚Ø±Ø©', 'Ø¢Ù„ Ø¹Ù…Ø±Ø§Ù†', 'Ø§Ù„Ù†Ø³Ø§Ø¡', 'Ø§Ù„Ù…Ø§Ø¦Ø¯Ø©', 'Ø§Ù„Ø£Ù†Ø¹Ø§Ù…', 'Ø§Ù„Ø£Ø¹Ø±Ø§Ù', 'Ø§Ù„Ø£Ù†ÙØ§Ù„', 'Ø§Ù„ØªÙˆØ¨Ø©', 'ÙŠÙˆÙ†Ø³', 'Ù‡ÙˆØ¯', 'ÙŠÙˆØ³Ù', 'Ø§Ù„Ø±Ø¹Ø¯', 'Ø¥Ø¨Ø±Ø§Ù‡ÙŠÙ…', 'Ø§Ù„Ø­Ø¬Ø±', 'Ø§Ù„Ù†Ø­Ù„', 'Ø§Ù„Ø¥Ø³Ø±Ø§Ø¡', 'Ø§Ù„ÙƒÙ‡Ù', 'Ù…Ø±ÙŠÙ…', 'Ø·Ù‡', 'Ø§Ù„Ø£Ù†Ø¨ÙŠØ§Ø¡', 'Ø§Ù„Ø­Ø¬', 'Ø§Ù„Ù…Ø¤Ù…Ù†ÙˆÙ†', 'Ø§Ù„Ù†ÙˆØ±', 'Ø§Ù„ÙØ±Ù‚Ø§Ù†', 'Ø§Ù„Ø´Ø¹Ø±Ø§Ø¡', 'Ø§Ù„Ù†Ù…Ù„', 'Ø§Ù„Ù‚ØµØµ', 'Ø§Ù„Ø¹Ù†ÙƒØ¨ÙˆØª', 'Ø§Ù„Ø±ÙˆÙ…', 'Ù„Ù‚Ù…Ø§Ù†', 'Ø§Ù„Ø³Ø¬Ø¯Ø©', 'Ø§Ù„Ø£Ø­Ø²Ø§Ø¨', 'Ø³Ø¨Ø£', 'ÙØ§Ø·Ø±', 'ÙŠØ³', 'Ø§Ù„ØµØ§ÙØ§Øª', 'Øµ', 'Ø§Ù„Ø²Ù…Ø±', 'ØºØ§ÙØ±', 'ÙØµÙ„Øª', 'Ø§Ù„Ø´ÙˆØ±Ù‰', 'Ø§Ù„Ø²Ø®Ø±Ù', 'Ø§Ù„Ø¯Ø®Ø§Ù†', 'Ø§Ù„Ø¬Ø§Ø«ÙŠØ©', 'Ø§Ù„Ø£Ø­Ù‚Ø§Ù', 'Ù…Ø­Ù…Ø¯', 'Ø§Ù„ÙØªØ­', 'Ø§Ù„Ø­Ø¬Ø±Ø§Øª', 'Ù‚', 'Ø§Ù„Ø°Ø§Ø±ÙŠØ§Øª', 'Ø§Ù„Ø·ÙˆØ±', 'Ø§Ù„Ù†Ø¬Ù…', 'Ø§Ù„Ù‚Ù…Ø±', 'Ø§Ù„Ø±Ø­Ù…Ù†', 'Ø§Ù„ÙˆØ§Ù‚Ø¹Ø©', 'Ø§Ù„Ø­Ø¯ÙŠØ¯', 'Ø§Ù„Ù…Ø¬Ø§Ø¯Ù„Ø©', 'Ø§Ù„Ø­Ø´Ø±', 'Ø§Ù„Ù…Ù…ØªØ­Ù†Ø©', 'Ø§Ù„ØµÙ', 'Ø§Ù„Ø¬Ù…Ø¹Ø©', 'Ø§Ù„Ù…Ù†Ø§ÙÙ‚ÙˆÙ†', 'Ø§Ù„ØªØºØ§Ø¨Ù†', 'Ø§Ù„Ø·Ù„Ø§Ù‚', 'Ø§Ù„ØªØ­Ø±ÙŠÙ…', 'Ø§Ù„Ù…Ù„Ùƒ', 'Ø§Ù„Ù‚Ù„Ù…', 'Ø§Ù„Ø­Ø§Ù‚Ø©', 'Ø§Ù„Ù…Ø¹Ø§Ø±Ø¬', 'Ù†ÙˆØ­', 'Ø§Ù„Ø¬Ù†', 'Ø§Ù„Ù…Ø²Ù…Ù„', 'Ø§Ù„Ù…Ø¯Ø«Ø±', 'Ø§Ù„Ù‚ÙŠØ§Ù…Ø©', 'Ø§Ù„Ø¥Ù†Ø³Ø§Ù†', 'Ø§Ù„Ù…Ø±Ø³Ù„Ø§Øª', 'Ø§Ù„Ù†Ø¨Ø£', 'Ø§Ù„Ù†Ø§Ø²Ø¹Ø§Øª', 'Ø¹Ø¨Ø³', 'Ø§Ù„ØªÙƒÙˆÙŠØ±', 'Ø§Ù„Ø§Ù†ÙØ·Ø§Ø±', 'Ø§Ù„Ù…Ø·ÙÙÙŠÙ†', 'Ø§Ù„Ø§Ù†Ø´Ù‚Ø§Ù‚', 'Ø§Ù„Ø¨Ø±ÙˆØ¬', 'Ø§Ù„Ø·Ø§Ø±Ù‚', 'Ø§Ù„Ø£Ø¹Ù„Ù‰', 'Ø§Ù„ØºØ§Ø´ÙŠØ©', 'Ø§Ù„ÙØ¬Ø±', 'Ø§Ù„Ø¨Ù„Ø¯', 'Ø§Ù„Ø´Ù…Ø³', 'Ø§Ù„Ù„ÙŠÙ„', 'Ø§Ù„Ø¶Ø­Ù‰', 'Ø§Ù„Ø´Ø±Ø­', 'Ø§Ù„ØªÙŠÙ†', 'Ø§Ù„Ø¹Ù„Ù‚', 'Ø§Ù„Ù‚Ø¯Ø±', 'Ø§Ù„Ø¨ÙŠÙ†Ø©', 'Ø§Ù„Ø²Ù„Ø²Ù„Ø©', 'Ø§Ù„Ø¹Ø§Ø¯ÙŠØ§Øª', 'Ø§Ù„Ù‚Ø§Ø±Ø¹Ø©', 'Ø§Ù„ØªÙƒØ§Ø«Ø±', 'Ø§Ù„Ø¹ØµØ±', 'Ø§Ù„Ù‡Ù…Ø²Ø©', 'Ø§Ù„ÙÙŠÙ„', 'Ù‚Ø±ÙŠØ´', 'Ø§Ù„Ù…Ø§Ø¹ÙˆÙ†', 'Ø§Ù„ÙƒÙˆØ«Ø±', 'Ø§Ù„ÙƒØ§ÙØ±ÙˆÙ†', 'Ø§Ù„Ù†ØµØ±', 'Ø§Ù„Ù…Ø³Ø¯', 'Ø§Ù„Ø¥Ø®Ù„Ø§Øµ', 'Ø§Ù„ÙÙ„Ù‚', 'Ø§Ù„Ù†Ø§Ø³']
RECITERS_MAP = {'ÙŠØ§Ø³Ø± Ø§Ù„Ø¯ÙˆØ³Ø±ÙŠ':'Yasser_Ad-Dussary_128kbps', 'Ø§Ù„Ø´ÙŠØ® Ø¹Ø¨Ø¯Ø§Ù„Ø±Ø­Ù…Ù† Ø§Ù„Ø³Ø¯ÙŠØ³': 'Abdurrahmaan_As-Sudais_64kbps', 'Ø§Ù„Ø´ÙŠØ® Ù…Ø§Ù‡Ø± Ø§Ù„Ù…Ø¹ÙŠÙ‚Ù„ÙŠ': 'Maher_AlMuaiqly_64kbps', 'Ø§Ù„Ø´ÙŠØ® Ù…Ø­Ù…Ø¯ ØµØ¯ÙŠÙ‚ Ø§Ù„Ù…Ù†Ø´Ø§ÙˆÙŠ (Ù…Ø¬ÙˆØ¯)': 'Minshawy_Mujawwad_64kbps', 'Ø§Ù„Ø´ÙŠØ® Ø³Ø¹ÙˆØ¯ Ø§Ù„Ø´Ø±ÙŠÙ…': 'Saood_ash-Shuraym_64kbps', 'Ø§Ù„Ø´ÙŠØ® Ù…Ø´Ø§Ø±ÙŠ Ø§Ù„Ø¹ÙØ§Ø³ÙŠ': 'Alafasy_64kbps', 'Ø§Ù„Ø´ÙŠØ® Ù…Ø­Ù…ÙˆØ¯ Ø®Ù„ÙŠÙ„ Ø§Ù„Ø­ØµØ±ÙŠ': 'Husary_64kbps', 'Ø§Ù„Ø´ÙŠØ® Ø£Ø¨Ùˆ Ø¨ÙƒØ± Ø§Ù„Ø´Ø§Ø·Ø±ÙŠ': 'Abu_Bakr_Ash-Shaatree_128kbps', 'Ù†Ø§ØµØ± Ø§Ù„Ù‚Ø·Ø§Ù…ÙŠ':'Nasser_Alqatami_128kbps', 'Ù‡Ø§Ù†ÙŠ Ø§Ù„Ø±Ø§ÙØ¹ÙŠ':'Hani_Rifai_192kbps', 'Ø¹Ù„ÙŠ Ø¬Ø§Ø¨Ø±' :'Ali_Jaber_64kbps'}
...
def get_text(surah, ayah):
    try:
        ...
        # Ø§Ù„Ù…Ù†Ø·Ù‚ Ø§Ù„Ø¬Ø¯ÙŠØ¯ Ù„Ù…Ø³Ø­ Ø§Ù„Ø¨Ø³Ù…Ù„Ø© (Ù…Ø§ Ø¹Ø¯Ø§ Ø§Ù„ÙØ§ØªØ­Ø© ÙˆØ§Ù„ØªÙˆØ¨Ø©)
        if surah != 1 and surah != 9 and ayah == 1:
            # Ù†Ù…Ø· ÙŠØ¨Ø­Ø« Ø¹Ù† Ø§Ù„Ø¨Ø³Ù…Ù„Ø© ÙÙŠ Ø¨Ø¯Ø§ÙŠØ© Ø§Ù„Ù†Øµ ÙˆÙŠÙ…Ø³Ø­Ù‡Ø§ Ù…Ù‡Ù…Ø§ ÙƒØ§Ù† ØªØ´ÙƒÙŠÙ„Ù‡Ø§
            basmala_pattern = r'^Ø¨ÙØ³Ù’Ù…Ù [^ ]+ [^ ]+ [^ ]+' 
            t = re.sub(basmala_pattern, '', t).strip()
            
            # ØªØ£ÙƒÙŠØ¯ Ø¥Ø¶Ø§ÙÙŠ Ù„Ùˆ Ø§Ù„Ù€ API Ø¨Ø¹Øª Ø§Ù„Ø¨Ø³Ù…Ù„Ø© ÙƒÙƒÙ„Ù…Ø© ÙˆØ§Ø­Ø¯Ø© Ù…Ø´Ù‡ÙˆØ±Ø©
            t = t.replace("Ø¨ÙØ³Ù’Ù…Ù Ù±Ù„Ù„Ù‘ÙŽÙ‡Ù Ù±Ù„Ø±Ù‘ÙŽØ­Ù’Ù…ÙŽÙ°Ù†Ù Ù±Ù„Ø±Ù‘ÙŽØ­ÙÙŠÙ…Ù", "").strip()
...
def build_video_task(job_id, user_pexels_key, reciter_id, surah, start, end, quality, bg_query, fps):
    ...
    try:
        ...
        # âœ… NEW: Add Fade Out (Video & Audio) - 0.2s
...
# ==========================================
# ðŸ§¹ Automatic Garbage Collector
# ==========================================
                ...
                for folder in os.listdir(BASE_TEMP_DIR):
                    ...
                    if os.path.isdir(folder_path):
                        if current_time - os.path.getctime(folder_path) > 3600:
                            ...
                            print(f"ðŸ—‘ï¸ Auto-deleted old workspace: {folder}")
        except Exception as e:
